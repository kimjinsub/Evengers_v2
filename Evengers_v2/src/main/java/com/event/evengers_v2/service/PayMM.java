   ...
   private String makeHtml_memberpayList(ArrayList<EventPay> epList) {
      ...
      for (EventPay ep : epList) {
         ...
         sb.append( "<div class='payList' name='"+ep_code+"'>" 
               ...
               + "      <div class='payList3'><p>이벤트명:" + e.getE_name() + "</p>"
         ...
         try {
            if (refundAble.compareTo(format.parse(format.format(new Date()))) >= 0) {// 환불가능
               sb.append("<div id='"+ep_code+"'class='btnCss'><button class='ep_code' name='" + ep_code + "'>환불하기</button></div>");
            } else {// 환불불가
               sb.append("<div id='"+ep_code+"'class='btnCss'><button disabled=disabled>환불불가</button></div>");
            }
            // A.compareTo(B)
